...
"""
ThirdTest_SimRuns: Bubo vs Async vs JFR per (CompId, LoopId) comparison,
...
Features:
  ...
  - For each (CompId, LoopId), computes PERCENT CHANGE in raw units
    when going from baseline (no slowdown) to slowdown:
      ...
      * AsyncPercentChange = (samples_slow - samples_base)
                             / samples_base * 100
      * JfrPercentChange   = (samples_slow - samples_base)
                             / samples_base * 100
  ...
  - X tick labels show, for the SLOWDOWN run, each loop’s contribution
    according to Bubo, JFR, and Async:

      line 1: "B xx.x% / J yy.y% / A zz.z%"
...
# -----------------------------------------------------------------------------
# Async + JFR marker layout helpers
# -----------------------------------------------------------------------------
...
def iter_blocks(lines):
    """Yield (header_line, frame_lines) for async-profiler output."""
    i = 0
    n = len(lines)
    while i < n:
...
def extract_marker_ids_from_funcs(funcs):
    """
    Generic marker extractor used both for Async and JFR.

    funcs: list of strings, each describing one stack frame's "function part",
           e.g. 'jdk.graal...BuboAgentCompilerMarkers.Marker12(...)'

    Layout:

      ... Marker<digit> ...              (loop id frame)
      ... MarkerDelimiter ...            (delimiter frame)
      ... Marker<digit> ...              (comp-id digits frames)
      ...
...
def extract_marker_ids(frame_lines):
    """
    Async-profiler: frame_lines are like

      [0] jdk.graal...BuboAgentCompilerMarkers.Marker12
      [1] jdk.graal...BuboAgentCompilerMarkers.MarkerDelimiter
      ...

    We first strip the '[idx]' prefix, then delegate to extract_marker_ids_from_funcs.
    """
    funcs = []
...
# -----------------------------------------------------------------------------
# JFR parsing – from 'jfr print' style text dump
# -----------------------------------------------------------------------------

def parse_jfr_marker_file(path):
    """
    Parse a JFR text dump (produced via something like:
      jfr print --events jdk.ExecutionSample foo.jfr > foo_JFR.txt

    We treat each jdk.ExecutionSample's stackTrace block as ONE sample
    with weight 1. For each sample, we look for our marker frames:

        BuboAgentCompilerMarkers.Marker<digit>()
        BuboAgentCompilerMarkers.MarkerDelimiter()
        BuboAgentCompilerMarkers.Marker<digit>()
        ...

    and decode (comp_id, loop_id) using extract_marker_ids_from_funcs.

    Returns:
      total_samples: int (number of samples that had valid markers)
      results: dict[(comp_id, loop_id)] = samples
    """
    try:
        lines = path.read_text(encoding="utf-8", errors="replace").splitlines()
    ...
    def flush_stack():
        ...
        if not current_stack:
            return
        ids = extract_marker_ids_from_funcs(current_stack)
        if ids is not None:
    ...
    for line in lines:
        ...
        # Start of a new jdk.ExecutionSample – flush any previous stack
        ...
        if in_stack:
            ...
            #   ...
            if s:
                # Remove trailing commas if any (some JFR formats use them)
                frame = s.rstrip(",")
...
def create_benchmark_plot(
    ...
    k = len(rows)
    x = list(range(k))
    width = 0.25  # 3 bars per group

    bubo_changes = [r["BuboPercentChange"] for r in rows]
    jfr_changes = [r["JfrPercentChange"] for r in rows]
    async_changes = [r["AsyncPercentChange"] for r in rows]

    fig_width = max(10, k * 0.8)
    fig, ax = plt.subplots(figsize=(fig_width, 6))

    x_bubo = [i - width for i in x]
    x_jfr = [i for i in x]
    x_async = [i + width for i in x]
    ...
    for r in rows:
        ...
        j_slow_share = r.get("JfrSlowdownSharePercent", 0.0)
        a_slow_share = r.get("AsyncSlowdownSharePercent", 0.0)

        line1 = f"B {b_slow_share:.1f}% / J {j_slow_share:.1f}% / A {a_slow_share:.1f}%"
    ...
    ax.set_xlabel(
        "Compilation unit + loop id\n"
        "(label shares use slowdown run)"
    )

    title = f"{bench_name} – Bubo vs JFR vs Async: percent change in raw units"
    ax.set_title(title)
...
def analyze_benchmark(bench_dir: Path, root_out: Path):
    ...
    # JFR text dumps (adjust names if yours differ)
    jfr_baseline_path = bench_dir / f"{bench_name}_LIR_false_JFR.txt"
    jfr_slowdown_path = bench_dir / f"{bench_name}_LIR_true_JFR.txt"
    ...
    if not jfr_baseline_path.exists():
        print(f"  [WARN] Missing {jfr_baseline_path.name}, JFR baseline data will be zero.")
    if not jfr_slowdown_path.exists():
        print(f"  [WARN] Missing {jfr_slowdown_path.name}, JFR slowdown data will be zero.")
    ...
    # -------------------------------------------------------------------------
    # 2) Bubo vs Async vs JFR per (CompId, LoopId)
    ...
    # JFR baseline
    jfr_total_base, jfr_baseline_map = parse_jfr_marker_file(jfr_baseline_path)
    # JFR slowdown
    jfr_total_slow, jfr_slow_map = parse_jfr_marker_file(jfr_slowdown_path)

    # Common keys: loops that both tools (Bubo & Async) see in the baseline,
    # and that have LoopCallCount == 0 (via bubo_baseline_map).
    # JFR is optional – if missing for some loops, those bars will be 0.
    common_keys = sorted(
    ...
    # - total slowdown Bubo/Async/JFR: for slowdown shares used in labels
    ...
    total_jfr_slow_all = sum(jfr_slow_map.get(k, 0) for k in common_keys)
    ...
    for i, (comp_id, loop_id) in enumerate(common_keys):
        ...
        # Async baseline / slowdown
        a_samples_base = async_baseline_map[(comp_id, loop_id)]
        a_samples_slow = async_slow_map.get((comp_id, loop_id), 0)

        # JFR baseline / slowdown (may be missing -> 0)
        j_samples_base = jfr_baseline_map.get((comp_id, loop_id), 0)
        j_samples_slow = jfr_slow_map.get((comp_id, loop_id), 0)
        ...
        if j_samples_base > 0:
            jfr_pct_change = (j_samples_slow - j_samples_base) / float(j_samples_base) * 100.0
        else:
            jfr_pct_change = 0.0
        ...
        if total_jfr_slow_all > 0:
            j_slow_share = (j_samples_slow / float(total_jfr_slow_all)) * 100.0
        else:
            j_slow_share = 0.0
    ...
    out_csv = root_out / f"{bench_name}_BuboJfrAsync_CompLoopShares.csv"
    ...
    out_png = root_out / f"{bench_name}_BuboJfrAsync_CompLoopShares.png"
